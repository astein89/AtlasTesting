...
- **Default SQLite file:** `atlas.db` → **`dc-automation.db`** (set `DB_PATH` or run `mv atlas.db dc-automation.db`; **`dc_automation.db`** is still used if present and the new name is absent — see `docs/MIGRATION_DC_AUTOMATION.md`).
