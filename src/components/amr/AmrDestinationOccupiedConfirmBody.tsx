...
export function AmrDestinationOccupiedConfirmFooterRetry() {
...
/** Rich `showConfirm` body when the first mission destination should be empty but Hyperion reports a pallet. */
export function AmrDestinationOccupiedConfirmBody({ destinationRef }: { destinationRef: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setPresent(next)
      if (next === false) setClearedNotice(true)
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-4">
      <p>
        First mission destination (<span className="font-mono">{ref || '—'}</span>) reports a pallet and should be empty
        before moving a pallet. Create this mission anyway?
      </p>
      {clearedNotice ? (
        <p className="rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-900 dark:text-emerald-100">
          Latest check reports this stand empty. Cancel and submit again to skip this warning, or use Retry or Create mission
          anyway below to start now.
        </p>
      ) : null}
