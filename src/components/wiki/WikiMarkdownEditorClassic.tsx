...
import {
  persistWikiEditorShowPreview,
  readInitialWikiEditorShowPreview,
} from '@/components/wiki/wikiMarkdownEditorPreviewPrefs'
import type { WikiMarkdownEditorLayout } from '@/components/wiki/wikiMarkdownEditorTypes'

const MAX_EDITOR_HISTORY = 80

export type { WikiMarkdownEditorLayout }
...
export function WikiMarkdownEditorClassic({
  value,
  onChange,
  disabled,
  ...
  const [showPreview, setShowPreview] = useState(readInitialWikiEditorShowPreview)
  ...
  const setPreview = (visible: boolean) => {
    persistWikiEditorShowPreview(visible)
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col">
      ...
      <div
        className={
          showPreview
            ? 'flex min-h-0 flex-1 flex-col gap-4 lg:flex-row lg:items-stretch lg:gap-4'
      ...
      >
        <div
          className={
            showPreview
              ? inModal
                ? 'flex min-h-[min(36vh,18rem)] w-full min-w-0 flex-1 flex-col'
          ...
          <textarea
            ...
            className={
              inModal
                ...
                : 'min-h-[22rem] w-full flex-1 resize-y rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm text-foreground outline-none ring-primary focus:ring-2 disabled:opacity-60 lg:min-h-[min(calc(100dvh-13rem),48rem)]'
            }
          ...
          <div
            className={
              inModal
                ...
                : 'wiki-editor-preview flex min-h-[22rem] w-full flex-1 overflow-auto rounded-lg border border-border bg-card p-4 lg:min-h-[min(calc(100dvh-13rem),48rem)] lg:w-[calc(50%-0.5rem)] lg:max-w-xl xl:max-w-2xl'
            }
            aria-label="Live preview"
          >
            {value.trim() ? (
              <MarkdownMdRtView content={value} onContentChange={commitChange} />
            ) : (
