...
async function countRolesWithStarExcluding(excludeSlug?: string): Promise<number> {
  const rows = (await db.prepare('SELECT slug, permissions FROM roles').all()) as Array<{
    slug: string
    permissions: string
  }>
  let n = 0
...
async function countUsersAssignedToRole(slug: string): Promise<number> {
  const fromAssignments = (await db
    .prepare(`SELECT COUNT(DISTINCT user_id) as c FROM user_roles WHERE role_slug = ?`)
...
router.get(
  ...
  asyncRoute(async (_req: AuthRequest, res) => {
    const rows = (await db.prepare('SELECT slug, label, permissions FROM roles ORDER BY slug').all()) as Array<{
      slug: string
      label: string
      permissions: string
    }>
...
router.post(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const existing = (await db.prepare('SELECT slug FROM roles WHERE slug = ?').get(slugRaw)) as
      | { slug: string }
      | undefined
    if (existing) {
    ...
    if (!perms.includes('*') && (await countRolesWithStarExcluding()) === 0) {
      return res.status(400).json({ error: AT_LEAST_ONE_STAR_MSG })
    }

    await db.prepare('INSERT INTO roles (slug, label, permissions) VALUES (?, ?, ?)').run(
      slugRaw,
      label,
      JSON.stringify(perms)
    )
    const row = (await db.prepare('SELECT slug, label, permissions FROM roles WHERE slug = ?').get(slugRaw)) as {
      slug: string
      label: string
      permissions: string
    }
...
router.put(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const existing = (await db.prepare('SELECT slug FROM roles WHERE slug = ?').get(slug)) as
      | { slug: string }
      | undefined
    if (!existing) {
    ...
    if (!perms.includes('*') && (await countRolesWithStarExcluding(slug)) === 0) {
      return res.status(400).json({ error: AT_LEAST_ONE_STAR_MSG })
    }
    ...
    if (label) {
      await db.prepare('UPDATE roles SET label = ?, permissions = ? WHERE slug = ?').run(
        label,
        permissionsJson,
        slug
      )
    } else {
      await db.prepare('UPDATE roles SET permissions = ? WHERE slug = ?').run(permissionsJson, slug)
    }

    const row = (await db.prepare('SELECT slug, label, permissions FROM roles WHERE slug = ?').get(slug)) as {
      slug: string
      label: string
      permissions: string
    }
...
router.delete(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    if (
      parseRolePermissions(row.permissions).includes('*') &&
      (await countRolesWithStarExcluding(slug)) === 0
    ) {
      return res.status(400).json({ error: AT_LEAST_ONE_STAR_MSG })
    }
    const n = await countUsersAssignedToRole(slug)
    if (n > 0) {
      return res.status(400).json({
        error: `Cannot delete: ${n} user(s) still have this role. Remove it from those users first.`,
      })
    }
    await db.prepare('DELETE FROM roles WHERE slug = ?').run(slug)
    res.status(204).send()
  })
)
