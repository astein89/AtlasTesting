...
async function insertRecordHistory(
  recordId: string,
...
router.get(
  '/:id/history',
  requirePermission('records.history'),
  asyncRoute(async (req: AuthRequest, res) => {
    const { id } = req.params
    const record = await db.prepare('SELECT id FROM test_runs WHERE id = ?').get(id)
...
router.put(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const existing = (await db.prepare('SELECT * FROM test_runs WHERE id = ?').get(id)) as {
      id: string
      test_plan_id: string
      run_at: string
      entered_by: string
    ...
    values.push(id)
    await db.prepare(`UPDATE test_runs SET ${updates.join(', ')} WHERE id = ?`).run(...values)
...
router.delete(
  ...
  asyncRoute(async (req: AuthRequest, res) => {
    ...
    const row = (await db.prepare('SELECT * FROM test_runs WHERE id = ?').get(id)) as {
      id: string
      data: string | null
      status: string
    } | undefined
    if (!row) return res.status(404).json({ error: 'Record not found' })
    await insertRecordHistory(id, 'deleted', row.data, row.status, null, null, req.user!.id)
    await db.prepare('DELETE FROM test_runs WHERE id = ?').run(id)
    res.status(204).send()
  })
)
