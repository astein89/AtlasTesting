...
function isFolderUnderAncestor(descendantId: string, ancestorId: string): boolean {
  ...
  while (cur) {
    ...
    seen.add(cur)
    const row = db.prepare('SELECT parent_id FROM file_folders WHERE id = ?').get(cur) as
...
function getFolderAclLayersFromDb(folderId: string | null): string[][] {
  const layers: string[][] = []
  ...
  while (cur) {
    if (seen.has(cur)) break
    seen.add(cur)
    const row = db.prepare('SELECT parent_id, allowed_role_slugs FROM file_folders WHERE id = ?').get(cur) as
      | { parent_id: string | null; allowed_role_slugs: string | null }
      | undefined
    if (!row) break
...
function folderChainAccessibleFromNode(
...
function storedFileAllowedForUser(row: StoredFileRow, user: NonNullable<AuthRequest['user']>): boolean {
  return storedFileAccessibleToUser(
    ...
    { id: user.id, roles: user.roles, permissions: user.permissions },
    getFolderAclLayersFromDb(row.folder_id)
  )
}
...
function listFilesInFolder(folderId: string | null, sortBy: string, order: string): StoredFileRow[] {
  const dir = order.toUpperCase() === 'ASC' ? 'ASC' : 'DESC'
  let orderSql: string
  switch (sortBy) {
    case 'name':
      orderSql = `sf.original_filename COLLATE NOCASE ${dir}`
      break
    case 'size':
      orderSql = `(sf.size_bytes IS NULL) ASC, sf.size_bytes ${dir}`
      break
    case 'type':
      orderSql = `COALESCE(sf.mime_type, '') COLLATE NOCASE ${dir}, sf.original_filename COLLATE NOCASE ASC`
      break
    case 'date':
    default:
      orderSql = `datetime(sf.created_at) ${dir}`
  }
  const sql = `
    ...
    WHERE (sf.folder_id IS ? OR (sf.folder_id IS NULL AND ? IS NULL))
    ORDER BY ${orderSql}
  `
  return db.prepare(sql).all(folderId, folderId) as StoredFileRow[]
}

function getFileRow(id: string): StoredFileRow | undefined {
  return db
    .prepare(
      `SELECT sf.id, sf.original_filename, sf.storage_filename, sf.mime_type, sf.size_bytes,
...
router.get('/folders/tree', requirePermission('module.files'), (req: AuthRequest, res) => {
  const rows = db
    .prepare(
      `SELECT id, parent_id, name, created_at, allowed_role_slugs, created_by
       FROM file_folders ORDER BY lower(name) COLLATE NOCASE ASC`
    )
    .all() as FileFolderRow[]
  const user = req.user!
  const filtered = rows.filter((r) => folderChainAccessibleFromNode(r.id, user))
  res.json(buildFolderTree(filtered))
})

router.get('/folders', requirePermission('module.files'), (req: AuthRequest, res) => {
...
router.get('/folders/:folderId/download', requirePermission('module.files'), (req: AuthRequest, res, next) => {
  void (async () => {
    try {
      const folderId = req.params.folderId
      const root = db.prepare('SELECT id, parent_id, name FROM file_folders WHERE id = ?').get(folderId) as
      ...
      const user = req.user!
      if (!folderChainAccessibleFromNode(folderId, user)) {
        res.status(404).json({ error: 'Not found' })
        return
      }
      ...
      while (folderQueue.length) {
        ...
        seenFolders.add(id)
        subtreeIds.push(id)
        const kids = db.prepare('SELECT id FROM file_folders WHERE parent_id = ?').all(id) as { id: string }[]
        for (const k of kids) folderQueue.push(k.id)
      }

      const folderRows = subtreeIds.length
        ? (db
            .prepare(
      ...
      const accessible = fileRows.filter((r) =>
        storedFileAllowedForUser(
          {
            id: r.id,
...
router.post('/folders', requirePermission('module.files'), requireFilesManage, (req: AuthRequest, res) => {
  ...
  const parentId =
    typeof parentRaw === 'string' && parentRaw.trim() ? parentRaw.trim() : null
  if (parentId) {
    const p = db.prepare('SELECT id FROM file_folders WHERE id = ?').get(parentId) as { id: string } | undefined
  ...
  if ('allowedRoleSlugs' in (req.body ?? {})) {
    ...
    const pr = db
      .prepare('SELECT allowed_role_slugs FROM file_folders WHERE id = ?')
      .get(parentId) as { allowed_role_slugs: string | null } | undefined
...
router.patch('/folders/:folderId', requirePermission('module.files'), requireFilesManage, (req: AuthRequest, res) => {
  const folderId = req.params.folderId
  const existing = db
  ...
  if ('parentId' in req.body) {
    ...
    if (pr === null || pr === '') {
      ...
      if (isFolderUnderAncestor(nextParent, folderId)) {
        return res.status(400).json({ error: 'Cannot move folder under its descendant' })
      }
      const p = db.prepare('SELECT id FROM file_folders WHERE id = ?').get(nextParent) as { id: string } | undefined
      if (!p) return res.status(400).json({ error: 'Parent folder not found' })
    }
  }
  ...
  try {
    db.prepare('UPDATE file_folders SET parent_id = ?, name = ?, allowed_role_slugs = ? WHERE id = ?').run(
      nextParent,
      nextName,
      nextSlugs,
      folderId
...
router.delete('/folders/:folderId', requirePermission('module.files'), requireFilesManage, (req: AuthRequest, res) => {
  const folderId = req.params.folderId
  const nChild = db.prepare('SELECT COUNT(*) as c FROM file_folders WHERE parent_id = ?').get(folderId) as {
    c: number
  }
  const nFiles = db.prepare('SELECT COUNT(*) as c FROM stored_files WHERE folder_id = ?').get(folderId) as {
    c: number
  }
  if ((nChild?.c ?? 0) > 0 || (nFiles?.c ?? 0) > 0) {
    return res.status(409).json({ error: 'Folder is not empty' })
  }
  const r = db.prepare('DELETE FROM file_folders WHERE id = ?').run(folderId)
...
router.get('/', requirePermission('module.files'), (req: AuthRequest, res) => {
  ...
  const rows = listFilesInFolder(folderId, sortBy, order)
  const user = req.user!
  const filtered = rows.filter((r) => storedFileAllowedForUser(r, user))
  res.json(filtered)
})
...
router.post(
  ...
  (req: AuthRequest, res) => {
    ...
    if (typeof body.folderId === 'string' && body.folderId.trim()) {
      ...
      const f = db.prepare('SELECT id FROM file_folders WHERE id = ?').get(folderId) as { id: string } | undefined
...
router.put('/:id', requirePermission('module.files'), (req: AuthRequest, res) => {
  ...
  if ('folderId' in req.body) {
    ...
    if (pr === null || pr === '') {
      ...
      folder_id = pr.trim()
      const f = db.prepare('SELECT id FROM file_folders WHERE id = ?').get(folder_id) as { id: string } | undefined
      if (!f) return res.status(400).json({ error: 'Folder not found' })
    }
  }
  ...
  const nextRoles =
    allowed_role_slugs !== undefined ? allowed_role_slugs : row.allowed_role_slugs
  const nextOriginal =
    original_filename !== undefined ? original_filename : row.original_filename
  const coercedInherit = row.inherit_folder_acl ?? 1
  const nextInherit =
    inherit_folder_acl !== undefined ? inherit_folder_acl : coercedInherit
  ...
  const out = getFileRow(id)
  res.json(out)
})

router.get('/:id/view', requirePermission('module.files'), (req: AuthRequest, res) => {
  const id = req.params.id
  const row = db.prepare('SELECT * FROM stored_files WHERE id = ?').get(id) as
  ...
  if (!storedFileAllowedForUser(viewRow, user)) {
    return res.status(404).json({ error: 'Not found' })
  }
  sendFileBytes(res, row, 'inline')
})

router.get('/:id/download', requirePermission('module.files'), (req: AuthRequest, res) => {
  const id = req.params.id
  const row = db.prepare('SELECT * FROM stored_files WHERE id = ?').get(id) as
    | (StoredFileRow & { storage_filename: string })
    | undefined
  ...
  if (!storedFileAllowedForUser(dlRow, user)) {
    return res.status(404).json({ error: 'Not found' })
  }
  sendFileBytes(res, row, 'attachment')
})

router.delete('/:id', requirePermission('module.files'), (req: AuthRequest, res) => {
  const id = req.params.id
  const row = db
